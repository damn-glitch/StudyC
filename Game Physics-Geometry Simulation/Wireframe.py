...
class Face:
    def __init__(self, first, second, third, colour=False):
        ...
        self.colour = (
        random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)) if not colour else colour.tuple()
    ...
    def addEdges(self, *edgelist, **kwargs):
        ...
        if 'index' in kwargs and kwargs['index'] == False:
            for edge in edgelist:
                ...
                if not any(
                        (stick.first.x, stick.first.y, stick.first.z, stick.second.x, stick.second.y, stick.second.z) ==
