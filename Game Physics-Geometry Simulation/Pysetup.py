...
class Game:
    ...
    def __init__(self, box, wireframes, screen, **kwargs):
        ...
        self.stateitems = {0: [
            Button('PLAY', lambda: self.changestate(), gameDisplay, evManager=self.evManager,
                   rect=Rect(620, 20, 160, 40)),
            Button('BUILD', lambda: self.builder(), gameDisplay, evManager=self.evManager, rect=Rect(620, 70, 160, 40)),
            Button('DESELECT', lambda: [SOUND_DESELECT.play(), self.build.clear()], gameDisplay,
                   evManager=self.evManager, rect=Rect(620, 120, 160, 40)),
            Button('CLEAR', lambda: [SOUND_CLEAR.play(), self.wireframes[0].clear(), self.build.clear()], gameDisplay,
                   evManager=self.evManager, rect=Rect(620, 170, 160, 40)),
            Button('SAVE', lambda: self.items.append(
                InputBox(180, 200, 440, 200, (255, 200, 0), self.save, 'Save file as:', gameDisplay,
                         evManager=self.evManager)), gameDisplay, evManager=self.evManager,
                   rect=Rect(610, 550, 60, 30)),
            Button('LOAD', lambda: self.items.append(
                InputBox(180, 200, 440, 200, (255, 200, 0), self.load, 'Load file:', gameDisplay,
                         evManager=self.evManager)), gameDisplay, evManager=self.evManager,
                   rect=Rect(680, 550, 70, 30)),
            ...
            1: [
                ...
                Button('CREATE', lambda: self.changestate(), gameDisplay, evManager=self.evManager,
                       rect=Rect(620, 20, 160, 40)),
                TextBoxEvent(650, 80, 100, 40, 'FOV', gameDisplay, evManager=self.evManager),
                Scroller(700, 130, 440, [40, 170], lambda x: setattr(self.camera, 'fov', math.radians(x)), gameDisplay,
        ...
        self.builditems = {0: [],
                           ...
                           3: [
                               ...
                               ColouredBox(630, 280, 140, 50, self.facecolour, gameDisplay, evManager=self.evManager,
                                           layer=-0.9),
                               ColouredBox(620, 270, 160, 270, (130, 205, 235), gameDisplay, evManager=self.evManager,
                                           layer=-0.8),
                               Scroller(660, 340, 180, [0, 255], lambda x: setattr(self.facecolour, 'x', x),
                                        gameDisplay, evManager=self.evManager),
                               Scroller(700, 340, 180, [0, 255], lambda x: setattr(self.facecolour, 'y', x),
                                        gameDisplay, evManager=self.evManager),
                               Scroller(740, 340, 180, [0, 255], lambda x: setattr(self.facecolour, 'z', x),
                                        gameDisplay, evManager=self.evManager)
                           ]}
    ...
    def save(self, text):
        ...
        try:
            ...
            globalTimer.registerItem(
                TextBoxEvent.from_text(Vectors.Vector2d(64, 64), 'Name the file something else'.format(text),
                                       gameDisplay,
    ...
    def load(self, text):
        try:
            ...
            SOUND_ERROR.play()
            globalTimer.registerItem(
                TextBoxEvent.from_text(Vectors.Vector2d(64, 64), 'The file {} does not exist'.format(text), gameDisplay,
                                       evManager=self.evManager), 60)
        else:
            ...
            globalTimer.registerItem(
                TextBoxEvent.from_text(Vectors.Vector2d(64, 64), '{} file loaded!'.format(text), gameDisplay,
                                       evManager=self.evManager), 60)
    ...
    def builder(self):
        ...
        if len(self.build) == 1:
            ...
            globalTimer.registerItem(
                TextBoxEvent.from_text(Vectors.Vector2d(64, 64), 'Node built', gameDisplay, evManager=self.evManager),
                60)
            ...
            globalTimer.registerItem(
                TextBoxEvent.from_text(Vectors.Vector2d(64, 64), 'Edge built', gameDisplay, evManager=self.evManager),
                60)
            ...
            SOUND_BUILD.play()
            self.wireframes[0].addNodes(*self.build)
            self.wireframes[0].addEdges([self.build[0], self.build[1]], [self.build[1], self.build[2]],
                                        [self.build[2], self.build[0]], index=False)
            self.wireframes[0].addFaces([self.build[0], self.build[1], self.build[2]], index=False,
                                        colour=self.facecolour)
            globalTimer.registerItem(
                TextBoxEvent.from_text(Vectors.Vector2d(64, 64), 'Face built', gameDisplay, evManager=self.evManager),
                60)
            ...
            SOUND_ERROR.play()
            globalTimer.registerItem(TextBoxEvent.from_text(Vectors.Vector2d(64, 64), 'Nothing selected', gameDisplay,
                                                            evManager=self.evManager), 60)
...
STATE_ITEMS = {0: [TextBoxEvent(60, 100, 400, 100, 'Geometry Physics simulator', gameDisplay, evManager=Controller),
                   Button('EXIT GAME', lambda: globalAchievements.save() or exit(), gameDisplay, evManager=Controller,
                          rect=Rect(60, 430, 270, 100)),
                   Scroller(400, 250, 280, [0, 1], lambda x: [sound.set_volume(x) for sound in Constants.ALL_SOUNDS],
                            gameDisplay, evManager=Controller),
...
STATE_ITEMS[0].append(
    Button('PLAY GAME', partial(change_state, 1), gameDisplay, evManager=Controller, rect=Rect(60, 210, 270, 100)))
STATE_ITEMS[0].append(
    Button('ACHIEVEMENTS', partial(change_state, 2), gameDisplay, evManager=Controller, rect=Rect(60, 320, 270, 100)))
STATE_ITEMS[2].append(
    Button('BACK', partial(change_state, 0), gameDisplay, evManager=Controller, rect=Rect(380, 100, 150, 100)))
