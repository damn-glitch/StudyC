...
class InputBox(Rect):
    ...
    def notify(self, event):
                ...
                if event.type == 'ENTER':
                    ...
                    if len(self.text) < 20:
                        ...
                        Constants.globalTimer.registerItem(
                            TextBoxEvent.from_text(Vector2d(64, 64), 'Too many characters', self.display,
        ...
        if isinstance(event, RenderEvent):
            ...
            lightcolour = (
            min(255, self.colour[0] * 1.1), min(255, self.colour[1] * 1.1), min(255, self.colour[2] * 1.1))
        ...
        if isinstance(event, MouseClick):
            if event.action == 'P':
                ...
                if self.holder.collidepoint(Vector2d(*pygame.mouse.get_pos())):
                    ...
                    self.textbox = TextBoxEvent.from_text(Vector2d(self.holder.x + self.holder.w + 10, self.holder.y),
                                                          '{0:.1f}'.format(self.getValue()), self.display,
                                                          evManager=self.evManager, layer=-2)
                    return True
