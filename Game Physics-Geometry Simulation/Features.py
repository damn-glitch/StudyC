        ...
        if isinstance(event, TickEvent):
            ...
            if self.collidepoint(Vector2d(*pygame.mouse.get_pos())):
                ...
                for i in range(Constants.FIREWORK_PARTS):
                    ...
                    self.maker.items.append(
                        Firework(Vector2d(self.pos.x, self.pos.y), 10, Vector2d(cos(i), sin(i)) * randint(2, 5),
                                 self.colour, self.display, evManager=self.evManager, layer=self.layer))
                return True
...
class Firework(CircleToy):
    ...
    def __init__(self, pos, radius, velocity, colour, display, **kwargs):
        ...
        self.colour = (median([0, colour[0] + randint(-50, 50), 255]), median([0, colour[1] + randint(-50, 50), 255]),
                       median([0, colour[2] + randint(-50, 50), 255]))
    ...
    def notify(self, event):
        ...
        if isinstance(event, RenderEvent):
            ...
            pygame.draw.circle(self.display, self.colour, (int(self.pos.x), int(self.pos.y)),
                               int(self.radius * self.lifetime / self.maxlife))
    ...
    def notify(self, event):
        if isinstance(event, TickEvent):
            ...
            if randint(0, 30) == 0:
                ...
                self.items.append(
                    CircleToy(Vector2d(randint(0, 800), -radius), radius, self.display, self, evManager=self.evManager))
        ...
        if isinstance(event, TickEvent):
            ...
            if self.collidepoint(Vector2d(*pygame.mouse.get_pos())):
                if not self.textbox:
                    ...
                    self.textbox = TextBoxEvent.from_text(Vector2d(pygame.mouse.get_pos()[0], self.y - 40),
                                                          '{0:.2f} / {1} {2}'.format(*self.values), self.surface,
                ...
                if box.text == 'POPPER':
                    ...
                    if self.tracker['POPPER'] >= self.ideals['POPPER'] and not self.achievements['POPPER']:
                        ...
                        Constants.globalTimer.registerItem(
                            TextBoxEvent.from_text(Vector2d(64, 64), 'Achievement unlocked: POPPER', self.display,
                ...
                for box in self.boxes:
                    if box.text == 'SPEED':
                        ...
                        if event.velocity >= self.ideals['SPEED'] and not self.achievements['SPEED']:
                            ...
                            Constants.globalTimer.registerItem(
                                TextBoxEvent.from_text(Vector2d(64, 64), 'Achievement unlocked: SPEED', self.display,
