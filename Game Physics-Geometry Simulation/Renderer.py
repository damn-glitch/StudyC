...
class Screen:
    def __init__(self, camera, display, box):
        ...
        self.grid = Wireframe()
        self.grid.addNodes(*[Verlet(x, y, z, 0) for x in range(0, box.x + 1, Constants.REND_PREC) for y in
                             range(0, box.y + 1, Constants.REND_PREC) for z in
                             range(0, box.z + 1, Constants.REND_PREC)])
    ...
    def get3dPoint(self, point, bearing):
        ...
        if bearing:
            ...
            highest = [False, False]
            for node in [Vector3d(x, y, z) for x in range(int(bearing.x - 1), int(bearing.x + 2)) for y in
                         range(int(bearing.y - 1), int(bearing.y + 2)) for z in
                         range(int(bearing.z - 1), int(bearing.z + 2))]:
                proj = self.camera.renderP(node)
                dist = Vector2d.distance(proj, point)
    ...
    def render(self, grid, others):
        ...
        dup = list(
            allitems)  # duplicates items, to seperate the items from the grid that is going to be added to the orginal list
        if grid:
...
class Camera(object):
    ...
    def renderP(self, point):
        ...
        try:
            ...
            rotated.z = cos(self.theta.x) * (cos(self.theta.y) * relative.z + sin(self.theta.y) * (
                        sin(self.theta.z) * relative.y + cos(self.theta.z) * relative.x)) - sin(self.theta.x) * (
                                    cos(self.theta.z) * relative.y - sin(self.theta.z) * relative.x)
            ...
            rotated.x = cos(self.theta.y) * (sin(self.theta.z) * relative.y + cos(self.theta.z) * relative.x) - sin(
                self.theta.y) * relative.z
            rotated.y = sin(self.theta.x) * (cos(self.theta.y) * relative.z + sin(self.theta.y) * (
                        sin(self.theta.z) * relative.y + cos(self.theta.z) * relative.x)) + cos(self.theta.x) * (
                                    cos(self.theta.z) * relative.y - sin(self.theta.z) * relative.x)
